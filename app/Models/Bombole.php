<?php
	...
	class Bombole extends AlertBase
	{
		...
		public static function index($search = null, $order = 'livello', $page = 1, $slice = true) {
			...
			$query = Bombole::select([
				'bombole.id',
				'bombole.id_veicolo',
				'bombole.anno',
				'bombole.data_pagamento',
				'bombole.inizio_validita',
				'bombole.fine_validita',
				'bombole.importo',
				...
				->leftJoin('dettaglio_veicolo', 'bombole.id_veicolo', '=', 'dettaglio_veicolo.id')
				->leftJoin('targa', 'bombole.id_veicolo', '=', 'targa.id_veicolo')
				...
				->where(function($query) use ($search) {
					$query->where('bombole.id', 'LIKE', "%{$search}%")
						->orWhere('bombole.id_veicolo', 'LIKE', "%{$search}%")
						->orWhere('bombole.anno', 'LIKE', "%{$search}%")
						->orWhere('bombole.data_pagamento', 'LIKE', "%{$search}%")
						->orWhere('bombole.inizio_validita', 'LIKE', "%{$search}%")
						->orWhere('bombole.fine_validita', 'LIKE', "%{$search}%")
						->orWhere('bombole.importo', 'LIKE', "%{$search}%")
